...
class IOMicRecordStopCommand extends AbstractIOCommand {
   ...
   */
  Future _downSample(int targetSampleRate, int sourceSampleRate, Float32List sampleBuffer) {
    OfflineAudioContext o = new OfflineAudioContext(1, sampleBuffer.length, targetSampleRate);
  ...
  ByteData _encodeWAV(int numChannels, int sampleRate, Float32List interleaved){
    ...
    view.setUint16(22, numChannels, Endianness.LITTLE_ENDIAN);
    view.setUint32(24, sampleRate, Endianness.LITTLE_ENDIAN);
    view.setUint32(28, sampleRate * 4, Endianness.LITTLE_ENDIAN);
    view.setUint16(32, numChannels * 2, Endianness.LITTLE_ENDIAN);
  ...
  void _finalize(ByteData result) {
    ...
    html.Event event = new html.Event("click", canBubble: true, cancelable: true);
